...
def modify_multiple_positions_per_company(data_model):
    ...
    while i < len(x):
        start = i
        current_company = x[i].company
        current_start_date = x[i].start_date
        current_end_date = x[i].end_date
        
        # Find contiguous sequence with the same company
        while i < len(x) and x[i].company == current_company:
            i += 1

        if i > 0: current_start_date = x[i-1].start_date
        ...
        if count > 1:
            ...
            tmp_entry.is_position_in_multi_position_company = False
            x.insert(start, tmp_entry)
            ...
            # Adjust index to avoid reprocessing
            i = start + 1 + count

    data_model.cv.sections_input['experience'] = x
    return data_model
